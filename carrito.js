...
$(document).ready(function () {
    ...
    //BOTON SUBSCRIBIR
    $("#boton").prepend(
        " <a href='./views/contacto.html'><button class='btn btn-warning' id='btnSuscrip'>Suscribir a pratto</button></a>"
    );
...
function renderizarProductos() {
    for (const producto of productosJSON) {
        $(".milista")
            .append(`<li class="col-sm-3 cards centrarTxt borderNegro list-group-item">
        <h3 style="display: none"> ID: ${producto.id} </h3>
        <p class='boxmodCentrar kitchen' > - ${producto.nombre} -</p>
        <div class="boxmodCentrar">
            <img
                src="${producto.foto}"
                width="250"
                height="200"
...
function agregarAlCarrito(productoNuevo) {
    ...
    if (encontrado == undefined) {
        ...
        $(`#elimina${productoAAgregar.id}`).click(function () {
            console.log("ENtre");
            let posEliminar = carrito.findIndex(
                (p) => p.id == productoAAgregar.id
            );
...
const obtenerValorDolar = () => {
    ...
    $.ajax({
        ...
        success: function (data) {
            $("#fila_prueba").append(
                `<p class='boxmodCentrar colorBlanco centrarTxt'>Dolar compra:$ <b>${data.compra}</b> Dolar venta:$ <b>${data.venta}</b><p>`
            );
            console.log(data);
